var async = require('async')
  ...
  , util = require('util')
  , In = require('stream-tee').In
;

function TwitterStreamer(name, args) {
  'use strict';
  // Run super constructor
  In.apply(this, [name]);

  // Create twitter instance
  if (!args.auth.consumer_key
    || !args.auth.consumer_secret
    || !args.auth.access_token_key
    || !args.auth.access_token_secret )
  {
    throw new Error('Twitter API keys not entered');
  }
...
util.inherits(TwitterStreamer, In);

TwitterStreamer.prototype.stream = function() {
  'use strict';
  var _this = this;

(function($) {
    ...
    // Sent when the node app first connects (or reconnects)
    socket.on('clear_all', function() {
        // Clear arrays
        main_tweets = [];
        filtered_tweets.regex = '';

        socket.emit('need_tweets', {});

        // Update Herald articles
        update_herald();
    });

    // Tweets fetched from the database are handled differently, not
    // being drawn until they've all arrived.
    socket.on('first_tweet', function(data) {
        main_tweets.push(data);
    });

    socket.on('first_tweets_done', function() {
        main_tweets = main_tweets.sort(function(a, b) {
            return a.time - b.time;
        });
        change_filter("", function() {
            // 2000 is something of a magic (but not really) number
            setTimeout(show_streams, 2000);
        });
    });
    ...
    socket.on('tweet', function(data) {
        main_tweets.push(data);
        if (data.text.match(filtered_tweets.regex)) {
            make_post(data, streams[data.event], 'Twitter', false);
        }
    });
